...
| Pago rechazado (`rejected`) | Orden pasa a `fallido` — lógica implementada, comparte el mismo código verificado para `approved`. **No se pudo reproducir en vivo**: probando con la tarjeta de rechazo (titular `OTHE`) en múltiples sesiones nuevas, con distintos carritos e incluso con una cuenta de comprador de prueba recién creada, Mercado Pago devolvió consistentemente `in_process` en vez de `rejected` (confirmado inspeccionando el campo `estadoPago` logueado en `/api/webhook`). El código maneja este caso de forma segura: al no ser `approved`/`rejected`/`cancelled`, la orden queda en `pendiente` (fail-safe), en vez de asumir un estado incorrecto |
