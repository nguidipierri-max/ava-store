...
| Pago rechazado (`rejected`) | Orden pasa a `fallido` y se lo notifica al usuario en pantalla — **verificado end-to-end**. Nota de troubleshooting: inicialmente la tarjeta de rechazo (titular `OTHE`) devolvía `in_process` en vez de `rejected` de forma consistente. La causa fue un email de `payer` hardcodeado en la preferencia de pago que no coincidía con el comprador real logueado en el checkout; el desajuste entre el comprador declarado y el comprador autenticado disparaba una revisión manual del lado de Mercado Pago en vez de resolver el estado simulado de la tarjeta de prueba. Se sacó ese campo hardcodeado (dejando que Mercado Pago tome el email real de la sesión del comprador), y el caso quedó verificado en producción |
...
- Reproducir en un ambiente más estable el caso de pago cancelado (`cancelled`) de punta a punta — comparte código con `rejected`, ya verificado.
